...
import { FAQ_SECTION, HOME_FAQ, type FaqItem } from "@/content/faq";
        ...
        <Accordion.Item key={item.id} value={item.id} className="border-b border-edge">
          <Accordion.Header>
            <Accordion.Trigger className="group flex w-full items-start justify-between gap-6 py-6 text-left">
              ...
              <span
                aria-hidden="true"
                className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-pill border border-edge-strong text-ink-muted transition-transform duration-300 group-data-[state=open]:rotate-45"
              >
...
/** The homepage band: five questions and the way to the rest. */
export function Faq() {
  return (
    <Section id="faq" width="narrow" ruled>
      ...
      <p className="mt-8">
        <Link
          href={FAQ_SECTION.moreHref}
          className="text-apricot-ink underline decoration-1 underline-offset-4 hover:decoration-2"
        >
          {FAQ_SECTION.moreLabel}
        </Link>
      </p>
    </Section>
  );
